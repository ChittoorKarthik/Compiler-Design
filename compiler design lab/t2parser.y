...
telugu_function_name:   // only for functions being declared
    TELUGU_IDENTIFIER {printf("parser saw teluguFuncNamex");
...
// |   telugu_int telugu_arithmetic_operator telugu_int {
...
|   telugu_identifier telugu_open_square_bracket exp {registers[regstackpointer++]=registerIndex;
        sprintf(icg[ic_idx++], "MOV R%d , %s + R%d\n", registerIndex , $1.name, registerIndex-1);} telugu_closed_square_bracket { 
        printf("Parser found id[exp]\n");
        int num_children = 4;  // Number of children
        struct node **children = (struct node **)malloc(num_children * sizeof(struct node *));
        children[0] = $1.nd;
...
if_statement:
    telugu_if telugu_open_curly_bracket condition {
        sprintf(icg[ic_idx++], "if NOT (R%d) GOTO L%d\n",registerIndex-1,labelsused);isleader[insNumOfLabel[labelsused]]=1;isleader[ic_idx]=1; label[stackpointer++]=labelsused++;} telugu_closed_curly_bracket telugu_open_floor_bracket bunch_of_statements {sprintf(icg[ic_idx++], "JUMP L%d\n",label[stackpointer-1]);} telugu_closed_floor_bracket { 
...
while_loop:
    telugu_while telugu_open_curly_bracket condition { looplabel[looplabelstackpointer++] = labelsused; insNumOfLabel[labelsused]=ic_idx;  sprintf(icg[ic_idx++], "LABEL L%d:\n", labelsused++);
    ...
    } telugu_closed_curly_bracket telugu_open_floor_bracket bunch_of_statements { sprintf(icg[ic_idx++], "JUMPtoLOOP L%d\n",looplabel[--looplabelstackpointer]);
    } telugu_closed_floor_bracket {insNumOfLabel[gotolabel[gotolabelstackpointer-1]]=ic_idx;  sprintf(icg[ic_idx++], "LABEL L%d:\n", gotolabel[--gotolabelstackpointer]);} { 
...
variable_declaration:
    telugu_datatype telugu_identifier_declaring telugu_assignment_operator {rangestart = ic_idx;} exp {
...
function_declaration:
    telugu_function {oldscope=scope;scope=0;} telugu_function_name {add('F');scope=oldscope;} telugu_open_curly_bracket parameters_line telugu_closed_curly_bracket telugu_open_floor_bracket function_content telugu_closed_floor_bracket { 
        printf("Parser found equation\n");
